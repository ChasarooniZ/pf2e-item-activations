// Import groups and constants
import { generateActivations } from "./generate-activation.js";
import { handlePropertyRunes } from "./handle-property-runes.js";
import { ITEM_LIST, ITEM_SLUGS } from "./item-list.js";
import { setModuleFlag } from "./misc.js";
import { MODULE_ID } from "./const.js";
import { augmentAction } from "./on-create.js";

// Constant marker
export const marker = "[X]";

/**
 * Activates an action
 * @param {object} action The action to activate
 * @returns {object} The activated action
 */
export function activateAction(action) {
    setModuleFlag(action, "enabled", true);
    action.name = action.name.replaceAll(marker, "").trim();
    return action;
}

/**
 * Deactivates an action
 * @param {object} action The action to deactivate
 * @returns {object} The deactivated action
 */
export function deactivateAction(action) {
    setModuleFlag(action, "enabled", false);
    action.name = marker.concat(" ", action.name.replaceAll(marker, "").trim());
    return action;
}

/**
 * Turns on or off activations for a particular item
 * @param {object} item The item to toggle activation for
 * @param {'On' | 'Off' | 'None'} changeType Type of change (On, Off, None)
 */
export async function turnOnOffActivation(item, changeType) {
    const actor = item.actor;
    const id = item.id;

    let existingActions = actor.items.filter((it) => it?.flags?.[MODULE_ID]?.grantedBy._id === id);

    if (changeType === "On") {
        if (existingActions.length === 0) {
            await createMissingActivations(item, actor);
        }

        const actions = existingActions || actor.items.filter((it) => it?.flags?.[MODULE_ID]?.grantedBy._id === id);

        const nameIds = actions.map((action) => ({
            _id: action.id,
            name: activateAction(action).name,
            flags: {
                [MODULE_ID]: {
                    enabled: true,
                },
            },
        }));

        await actor.updateEmbeddedDocuments("Item", nameIds);
    } else if (changeType === "Off") {
        await actor.updateEmbeddedDocuments(
            "Item",
            existingActions.map((action) => ({
                _id: action._id,
                name: deactivateAction(action).name,
                flags: {
                    [MODULE_ID]: {
                        enabled: false,
                    },
                },
            }))
        );
    }
}
async function createMissingActivations(item, actor) {
    const activations = [];
    const slug = item.system.slug;

    if (ITEM_SLUGS.includes(slug)) {
        // Custom Built
        for (const actionUuid of ITEM_LIST[slug].actions) {
            let action = await fromUuid(actionUuid);
            action = action.toObject();
            action = augmentAction(action, item);
            activations.push(action);
        }
    } else {
        // Autogenerated
        activations.push(...generateActivations(item).map((act) => augmentAction(act, item)));
    }
    //Property Runes
    const { actives = [], rules = [] } = (await handlePropertyRunes(item)) || {};
    activations.push(...actives);
    if (rules.length > 0) {
        const currentRuleRunes = new Set(
            item.system.rules.filter((r) => r?.flags?.grantedBy?.uuid === item.uuid).map((r) => r?.flags?.rune)
        );
        const rulesToAdd = rules.filter((r) => !currentRuleRunes.has(r.flags.rune));
        if (rulesToAdd.length > 0) {
            await actor.updateEmbeddedDocuments("Item", [
                {
                    _id: item.id,
                    system: { rules: [...item.system.rules, ...rulesToAdd] },
                },
            ]);
        }
    }

    await actor.createEmbeddedDocuments("Item", activations);
}
